...
class EditSipDevice extends EditRecord
{
    protected static string $resource = SipDeviceResource::class;
    protected function getHeaderActions(): array { return [DeleteAction::make()]; }
    protected function mutateFormDataBeforeSave(array $data): array
    {
        $params = json_decode($this->record->dir_params ?? '{}', true) ?: [];
        if (!empty($data['sip_password'])) $params['password'] = $data['sip_password'];
        $params['vm-enabled']  = $data['vm_enabled'] ?? 'true';
        if (isset($data['vm_password'])) $params['vm-password'] = $data['vm_password'];
        if (isset($data['vm_mailto']))   $params['vm-mailto']   = $data['vm_mailto'];
        ...
        $vars = json_decode($this->record->dir_vars ?? '{}', true) ?: [];
        if (isset($data['effective_caller_id_name']))   $vars['effective_caller_id_name']   = $data['effective_caller_id_name'];
        if (isset($data['effective_caller_id_number'])) $vars['effective_caller_id_number'] = $data['effective_caller_id_number'];
