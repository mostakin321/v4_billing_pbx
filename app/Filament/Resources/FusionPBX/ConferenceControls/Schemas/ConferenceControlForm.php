...
namespace App\Filament\Resources\FusionPBX\ConferenceControls\Schemas;
...
class ConferenceControlForm
{
    public static function configure(Schema $form): Schema
    {
        return $form->schema([

            Tabs::make('Conference Control')
                ->columnSpanFull()
                ->tabs([
                    Tabs\Tab::make('Main')
                    ...
                    TextInput::make('control_name')
                        ->label('Control Name')->placeholder('control name'),
                    Select::make('control_enabled')
                        ->label('Control Enabled')
                        ->native(false)->options(['true'=>'Yes','false'=>'No'])->default('false'),
                    Textarea::make('control_description')
                        ->label('Control Description')->rows(2)->columnSpanFull(),
                                ]),
                        ]),
                ]),
