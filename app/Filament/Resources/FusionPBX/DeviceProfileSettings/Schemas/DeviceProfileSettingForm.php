...
namespace App\Filament\Resources\FusionPBX\DeviceProfileSettings\Schemas;
...
class DeviceProfileSettingForm
{
    public static function configure(Schema $form): Schema
    {
        return $form->schema([

            Tabs::make('Device Profile Setting')
                ->columnSpanFull()
                ->tabs([
                    Tabs\Tab::make('Main')
                        ...
                        ->schema([
                            Section::make('Main')
                                ->columns(2)
                                ->schema([
                    TextInput::make('profile_setting_name')
                        ->label('Profile Setting Name')->placeholder('profile setting name'),
                    TextInput::make('profile_setting_value')
                        ->label('Profile Setting Value')->placeholder('profile setting value'),
                    Select::make('profile_setting_enabled')
                        ->label('Profile Setting Enabled')
                        ->native(false)->options(['true'=>'Yes','false'=>'No'])->default('false'),
                    Textarea::make('profile_setting_description')
                        ->label('Profile Setting Description')->rows(2)->columnSpanFull(),
                                ]),
                        ]),
                ]),
