...
namespace App\Filament\Resources\FusionPBX\ContactUrls\Schemas;
...
class ContactUrlForm
{
    public static function configure(Schema $form): Schema
    {
        return $form->schema([

            Tabs::make('Contact Url')
                ->columnSpanFull()
                ->tabs([
                    Tabs\Tab::make('Main')
                        ...
                        ->schema([
                            Section::make('Main')
                                ->columns(2)
                                ->schema([
                    TextInput::make('url_type')
                        ->label('Url Type')->placeholder('url type'),
                    TextInput::make('url_label')
                        ->label('Url Label')->placeholder('url label'),
                    TextInput::make('url_primary')
                        ->label('Url Primary')->placeholder('url primary'),
                    TextInput::make('url_address')
                        ->label('Url Address')->placeholder('url address'),
                    Textarea::make('url_description')
                        ->label('Url Description')->rows(2)->columnSpanFull(),
                                ]),
                        ]),
                ]),
